...
ZH_CONTRIBUTION_FIXES = {
    "Improve event connection handling and reconnection logic": "改进事件连接生命周期管理与重连逻辑，提升断线后的恢复稳定性",
}

EN_CONTRIBUTION_FIXES = {
    "Improve event connection handling and reconnection logic": "Improved event connection lifecycle handling and reconnection logic to make recovery after disconnects more reliable",
}

GENERATED_NOTICE = "<!-- AUTO-GENERATED by scripts/render_profile_readme.py. Edit profile-data.json for curated project notes. -->"
PROJECT_TABLE_FILLER = "&nbsp;" * 36
CONTRIBUTION_TABLE_FILLER = "&nbsp;" * 96
...
def render_contributions(lines: list[str], contributions: list[dict[str, Any]], lang: str) -> None:
    ...
    for area in ordered_areas:
        lines.extend([
            ...
            f'      <th align="left" width="62%">What I Fixed{CONTRIBUTION_TABLE_FILLER}</th>',
            "    </tr>",
            "  </thead>",
            "  <tbody>",
        ])
...
def render_projects(lines: list[str], projects: list[dict[str, Any]], lang: str) -> None:
    lines.extend([
        ...
        f'      <th align="left" width="52%">Notes{PROJECT_TABLE_FILLER}</th>',
        "    </tr>",
        "  </thead>",
        "  <tbody>",
    ])
