...
ZH_CONTRIBUTION_FIXES = {
    ...
    "Fix test name detection for Git-style paths": "修复 Git 风格路径下的测试名称检测",
    ...
    "Render validation array indices with brackets": "将校验数组索引用方括号渲染",
    "Reject malformed embedding batch JSONL bytes": "拒绝格式错误的 embedding batch JSONL 字节",
    "Preserve reverse-proxy path prefixes": "保留反向代理路径前缀",
    "Python: accept AG-UI state data URI parameters": "Python：接受 AG-UI state data URI 参数",
    "Python: raise ValueError for malformed data URIs": "Python：对格式错误的 data URI 抛出 ValueError",
    ...
    "Recover vectors when payload id is missing": "在 payload id 缺失时恢复向量",
    "Avoid crashes on malformed Markdown hash links": "避免格式错误的 Markdown hash links 导致崩溃",
...
class RenderContext:
    previous_fixes: dict[tuple[str, str], str]
...
def plain_text_from_cell(markdown: str) -> str:
...
def contribution_cache_key(url: Any, lang: str) -> tuple[str, str] | None:
...
def extract_previous_contribution_fixes(content: str) -> dict[tuple[str, str], str]:
    ...
    for line in content.splitlines():
        if line.strip() == "---":
            current_lang = "zh"
            continue
        if "github.com/" not in line or "/pull/" not in line:
            continue
        cells = split_markdown_table_row(line)
        if len(cells) < 3:
            continue
        match = re.search(r"\]\((https://github\.com/[^)]+/pull/\d+)\)", cells[1])
        if not match:
            continue
        text = plain_text_from_cell(cells[2])
        if text:
            fixes[(current_lang, match.group(1))] = text
    return fixes


def load_previous_context(path: Path) -> RenderContext:
    if not path.exists():
        return RenderContext(previous_fixes={})
    return RenderContext(previous_fixes=extract_previous_contribution_fixes(path.read_text(encoding="utf-8")))
            ...
            lines.append(
                ...
                + " | ".join(
                    [
                        ...
                        md_left(contribution_fix_text(item, lang, context)),
                    ]
                )
                + " |"
            )
...
def render_section(lines: list[str], data: dict[str, Any], projects: list[dict[str, Any]], contributions: list[dict[str, Any]], lang: str, include_badges: bool, context: RenderContext) -> None:
...
def render(data: dict[str, Any], context: RenderContext | None = None) -> str:
    context = context or RenderContext(previous_fixes={})
    projects = enrich_projects(data)
    contributions = contribution_rows(data)
    validate_projects(projects)
    lines: list[str] = [GENERATED_NOTICE, ""]
    render_section(lines, data, projects, contributions, "en", include_badges=True, context=context)
    lines.extend(["", "---", ""])
...
def main() -> int:
    ...
    context = load_previous_context(args.out)
    write_output(args.out, render(data, context))
